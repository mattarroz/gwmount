...
static bool hdReadATA(void* h, uint8_t* ideregs, uint8_t* datap, int length) {
	uint32_t size = sizeof(ATA_PASS_THROUGH_EX) + length;
...
static bool hdGetMetaATA(void* h, bool atapi, uint8_t* datap) {
	uint32_t r, size;
...
static int doScsiIn(void* h, const uint8_t* cdb, int cdblen, uint8_t* in, int insize, bool fast) {
...
static bool getSignatureFromHandle(void* h, uint32_t& signature, uint32_t& partitionStyle) {
	uint32_t written;
	uint32_t outsize = sizeof(DRIVE_LAYOUT_INFORMATION_EX) + sizeof(PARTITION_INFORMATION_EX) * 32;
...
static int isMounted(void* hd) {
	int mounted;

	uint32_t signature, partitionStyle;
	...
	void* h = FindFirstVolume(volname, MAX_PATH);
	while (h != INVALID_HANDLE_VALUE && !mounted) {
		if (volname[wcslen(volname) - 1] == '\\') volname[wcslen(volname) - 1] = 0;
		void* d = CreateFile(volname, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (d != INVALID_HANDLE_VALUE) {
			uint32_t isntfs, outsize, written;
			isntfs = 0;
			if (DeviceIoControl(d, FSCTL_IS_VOLUME_MOUNTED, NULL, 0, NULL, 0, &written, NULL)) {
				...
				if (DeviceIoControl(d, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS, NULL, 0, vde, outsize, &written, NULL)) {
					for (uint32_t i = 0; i < vde->NumberOfDiskExtents; i++) {
						WCHAR pdrv[MAX_PATH];
						void* ph;
...
bool hdGetMetaHack(void* h, uint8_t* data, uint8_t* inq, CDriveList::CDevice& device) {
...
bool getStorageInfo(CDriveList::CDevice& drive, const STORAGE_DEVICE_NUMBER sdnp) {
	...
	uint32_t vpm[3] = { 0xffffffff , 0xffffffff , 0xffffffff };
	...
	for (;;) {
		...
		uint32_t dwRequiredSize, returnedLength;
		...
		void* hDev = CreateFile(pInterfaceDetailData->DevicePath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
bool readIdentity(void* h, CDriveList::CDevice& drive) {
...
static bool doScsiRead10CHS(void* handle, uint32_t lba, int c, int h, int s, uint8_t* data, int cnt, int* pflags, bool log) {
	uint8_t cmd[10];
	bool r;
	int flags = *pflags;
...
CDriveList::DangerType safetyCheck(void* h, const std::wstring& name, uint64_t offset, uint8_t* buf, int blocksize, uint8_t* identity, bool canCHS) {
...
void getDevicePropertyFromName(const std::wstring& devicePath, bool ignoreDuplicates, CDriveList::DeviceList& deviceList) {
	...
	void* hDevice = CreateFile(device->devicePath.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	...
	if (!DeviceIoControl(hDevice, IOCTL_STORAGE_QUERY_PROPERTY, &query, sizeof(STORAGE_PROPERTY_QUERY), &tmpBuffer[0], (uint32_t)tmpBuffer.size(), &returnedLength, NULL)) {
	...
	if (!device->isAccessible) {
		...
		if (DeviceIoControl(hDevice, IOCTL_DISK_GET_DRIVE_LAYOUT, NULL, 0, &tmpBuffer[0], (uint32_t)tmpBuffer.size(), &returnedLength, NULL)) {
			...
			if (dli->PartitionCount) {
				...
				for (uint32_t i = 0; i < dli->PartitionCount; i++) {
...
bool getDeviceProperty(HDEVINFO devInfo, uint32_t Index, CDriveList::DeviceList& deviceList) {
	SP_DEVICE_INTERFACE_DATA            interfaceData;
	PSP_DEVICE_INTERFACE_DETAIL_DATA    interfaceDetailData = NULL;
	BOOL                                status;
	uint32_t								reqSize, errorCode;
...
void CDriveAccess::closeDrive() {
	for (void* h : m_lockedVolumes) CloseHandle(h);
...
bool CDriveAccess::openDrive(const CDriveList::CDevice& device, bool readOnly, bool lockDrive) {
	...
	uint32_t flags = FILE_ATTRIBUTE_NORMAL | FILE_FLAG_RANDOM_ACCESS;
	...
	while (h != INVALID_HANDLE_VALUE) {
		...
		void* d = CreateFile(volName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (d != INVALID_HANDLE_VALUE) {
			if (DeviceIoControl(d, FSCTL_IS_VOLUME_MOUNTED, NULL, 0, NULL, 0, &written, NULL)) {
				NTFS_VOLUME_DATA_BUFFER ntfs;
				if (DeviceIoControl(d, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0, &ntfs, sizeof ntfs, &written, NULL)) isntfs = true;
				uint32_t outsize = sizeof(VOLUME_DISK_EXTENTS) + sizeof(DISK_EXTENT) * 32;
				...
				if (vde) {
					if (DeviceIoControl(d, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS, NULL, 0, vde, outsize, &written, NULL)) {
						for (uint32_t i = 0; i < vde->NumberOfDiskExtents; i++) {
							...
							WCHAR pdrv[MAX_PATH];
							swprintf_s(pdrv, L"\\\\.\\PhysicalDrive%d", vde->Extents[i].DiskNumber);
							void* ph = CreateFile(pdrv, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
							if (ph != INVALID_HANDLE_VALUE) {
								uint32_t sign2, pstyle2;
