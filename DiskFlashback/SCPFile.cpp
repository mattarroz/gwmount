...
bool SCPFile::readSCPFile() {
	...
	if (!ReadFile(m_file, trackOffsets.data(), (uint32_t)(sizeof(uint32_t) * trackOffsets.size()), &read, NULL)) read = 0;
...
// Flags from WINUAE
SCPFile::SCPFile(void* file, std::function<void(bool diskInserted, SectorType diskFormat)> diskChangeCallback)
