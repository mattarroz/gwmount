...
static int dfbRelease(struct AdfDevice* const dev) {
    free(dev);
    return ADF_RC_OK;
}


static int dfbReadSector(struct AdfDevice* const dev, const uint32_t n, const unsigned size, uint8_t* const buf) {
...
static int dfbWriteSector(struct AdfDevice* const dev, const uint32_t n, const unsigned size, const uint8_t* const    buf) {
    SectorCacheEngine* d = (SectorCacheEngine*)dev->drvData;
