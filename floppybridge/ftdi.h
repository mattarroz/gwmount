...
#define LPOVERLAPPED void*
	...
	public:
		...
		FT_STATUS FT_OpenEx(LPVOID pArg1, uint32_t Flags);

		FT_STATUS FT_ListDevices(LPVOID pArg1, LPVOID pArg2, uint32_t Flags);

		FT_STATUS FT_Close();

		FT_STATUS FT_Read(LPVOID lpBuffer, uint32_t nBufferSize, LPDWORD lpBytesReturned);
		FT_STATUS FT_Write(LPVOID lpBuffer, uint32_t nBufferSize, LPDWORD lpBytesWritten);

		FT_STATUS FT_IoCtl(uint32_t dwIoControlCode, LPVOID lpInBuf, uint32_t nInBufSize, LPVOID lpOutBuf, uint32_t nOutBufSize, LPDWORD lpBytesReturned, LPOVERLAPPED lpOverlapped);
		...
		FT_STATUS FT_SetEventNotification(uint32_t Mask,LPVOID Param);
		...
		FT_STATUS FT_GetDeviceInfoDetail(uint32_t dwIndex, LPDWORD lpdwFlags, LPDWORD lpdwType, LPDWORD lpdwID, LPDWORD lpdwLocId, char* pcSerialNumber, char* pcDescription, FT_HANDLE* handle);
